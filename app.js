...
// On regarde déjà si un compte admin n'existe pas
async function check(){
    const alreadyExistingAccount = await User.findOne({username: "admin"});
    if (alreadyExistingAccount !== null) {
        console.log("Un compte existe déjà avec cette adresse email");
    }
    else {
        // On va créer un utilisateur admin avec lequel on se connectera pour créer d'autres utilisateurs. Son mot de passe sera admin
        const adminUser = {
            nom: "admin",
            prenom: "admin",
            age: 22,
        ...
        createUser(adminUser).then((result) => {
